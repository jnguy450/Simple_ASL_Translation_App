...
extension VideoProcessingChain {
    ...
    /// - Returns: A `Pose` array if `VNDetecthumanHandPoseRequest` succeeds
    ...
    private func findPosesInFrame(_ frame: CGImage) -> [Pose]? {
        ...
        let poses = Pose.fromObservations(humanHandPoseRequest.results)
