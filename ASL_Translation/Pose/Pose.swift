...
/// Stores the landmarks and connections of a human hand pose and draws them as
/// a wireframe.
/// - Tag: Pose
struct Pose {
    /// The names and locations of the significant points on a human hand.
    ...
    /// Creates a `Pose` for each human hand pose observation in the array.
    /// - Parameter observations: An array of human hand pose observations.
    ...
    /// - Parameter observation: A human hand pose observation.
    init?(_ observation: Observation) {
        ...
        landmarks = observation.availableJointNames.compactMap { jointName in
            guard jointName != JointName.wrist else {
                return nil
            }
